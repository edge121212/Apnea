...
def process_segments_segment_level(base_dir, out_len=1024):
    ...
    # train/val records: a01~a20, b01~b05, c01~c10
    trainval_records = (
        [f"a{i:02d}" for i in range(1, 21)] +
        [f"b{i:02d}" for i in range(1, 6)] +
        [f"c{i:02d}" for i in range(1, 11)]
    )
    print("Train/Val records:", trainval_records)

    # test records: X01~X35
    test_records = [f"X{i:02d}" for i in range(1, 36)]
    print("Test records:", test_records)

    # 處理 train/val records
    train_data, val_data = [], []
    for rec_id in trainval_records:
        segs = preprocess_record(rec_id, base_dir, out_len=1024)
        n_segs = len(segs)
        n_val = int(np.ceil(0.2 * n_segs))
        n_train = n_segs - n_val
        train_data.extend(segs[:n_train])
        val_data.extend(segs[n_train:])
        print(f"{rec_id}: train={n_train}, val={n_val}")
